...
Um eine Inspektion der Daten vornehmen zu können, wird eine SQLite-Datenbank erstellt, indem beide Dateien in diese geladen werden:
...
##Allgemeine ÜBersicht der Daten

Mit folgendem Befehl wird eine Übersicht über die Vorhandenen Variablen ausgegeben:

```{sql connection=db}
SELECT * FROM pragma_table_info('penguins_raw');
```
...
FROM penguins_rawcount
GROUP BY studyName;
```

```{sql connection=db}
SELECT 'Sample Number', count('Sample Number')
FROM penguins_raw
...
```{sql connection=db}
SELECT 'Individual ID', COUNT('Individual ID')
FROM penguins_raw
